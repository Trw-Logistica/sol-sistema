...
const criar = async (req, res) => {
  const { nome, cliente_id, origem, destino, peso, produto, veiculo, numeros_whatsapp } = req.body;
...
const atualizar = async (req, res) => {
  const { id } = req.params;
  const campos = {};
  const fields = ['nome', 'cliente_id', 'origem', 'destino', 'peso', 'produto', 'veiculo', 'numeros_whatsapp'];
  for (const f of fields) {
    if (req.body[f] !== undefined) campos[f] = req.body[f] || (f === 'numeros_whatsapp' ? [] : null);
  }
